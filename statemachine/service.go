...
func StartMachine(sessionService *session.Service) *Service {
	machine = &Service{
...
func (w *Welcome) Receive(command Command, payload []byte) (DispatchReply, error) {
	...
	case LOAD:
		...
		sf, err := machine.sessionService.LoadSplitFile()
		if err != nil {
...
func (n *NewFile) Receive(command Command, payload []byte) (DispatchReply, error) {
	...
	case SUBMIT:
		...
		if err != nil {
			...
			splitFile, err := machine.sessionService.UpdateSplitFile(splitFilePayload)
...
func (r Running) Receive(command Command, params []byte) (DispatchReply, error) {
	...
	case SAVE:
		...
		err = machine.sessionService.SaveSplitFile(windowParams)
		if err != nil {
